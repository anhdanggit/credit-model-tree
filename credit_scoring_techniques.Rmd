...
By the data set from **Give Me Some Credit** (2012) <https://www.kaggle.com/c/GiveMeSomeCredit>, which contains 150,000 observations with 11 features. The nature of data set is unbalance, with only 7% of bad rate. According to this extreme unbalanced data, some other algorithms might be favoured (such as gradient boosting), yet this work is to use it to illustrate some useful techniques in Credit Scoring Modeling, namely:
...
library(vcd)
library(partykit)
library(CHAID) # install.packages("CHAID", repos="http://R-Forge.R-project.org")
...
```{r RevolvingUtilizationOfUnsecuredLines, echo=FALSE}
rank.plots(train_set %>% 
             select(RevolvingUtilizationOfUnsecuredLines),
          train_set$SeriousDlqin2yrs)
```
```{r MonthlyIncome, echo=FALSE}
rank.plots(train_set %>% 
...
rank.plots(train_set %>% 
             select(DebtRatio),
          train_set$SeriousDlqin2yrs, smooth = F)
```
...
```
```{r DebtRatio_plot, echo=F}
# check the rank.plots again
rank.plots(train_set %>% 
             select(DebtRatio),
          train_set$SeriousDlqin2yrs, smooth = T)
...
```
```{r chaid_graph, fig.height=18, fig.width=20}
...
```{r roc_curve, fig.width=20, fig.height=20}
...
plot(m4_perf, col='black',add=TRUE,lty=6); # CHAID Tuned
...
1. *Statistics Solutions*, CHAID: Non-parametric Analysis, <https://www.statisticssolutions.com/non-parametric-analysis-chaid/)> 
